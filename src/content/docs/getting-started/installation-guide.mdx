...
<Tabs>
  <TabItem label="Manual">
    <Steps>
    1. To install this way, first make a folder for your new project (which is going to include your new config file):
        ```bash
        mkdir <your-config-folder>
        ```
  ...
  <TabItem label="Fabric CLI">
    Alternatively, you can install Fabric through its CLI, you first need to the [Fabric CLI](https://github.com/Fabric-Development/fabric-cli) utility first.
    <Steps>
    1. To install this way, first make a folder for your new project (which is going to include your new config file):
        ```bash
        mkdir <your-config-folder>
        ```
    2. Change to this new folder:
        ```bash
        cd <your-config-folder>
        ```
    3. Create a new virtual environment:
        ...
        fabric-cli init
        ```
        An interactive dialog should appear, it will guide you from there...
    </Steps>
  </TabItem>
</Tabs>
    
_`<your-config-folder>` is common to be `~/.config/fabric`_
...
<Aside type="caution">
  Unless you are on NixOS, installing Fabric globally **is NOT recommended** and can easily cause issues
  as you're being tied to your system's distribution packages.
</Aside>
