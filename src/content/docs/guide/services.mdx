...
it's really easy to define a Service, here's a snippet that does exactly that...
...
2. **Using a Direct Argument in `Service` Constructor**
   Since we pass `**kwargs` over to the `super` which's a `Service`, You can also pass the callback directly as a keyword argument when initializing the `Service`, using `notify_<property_name_in_snake_case>` as the argument name.
